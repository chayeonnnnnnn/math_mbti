...
type_data = {
    "LDNB": {
        "name": "🧠📐 원리 장인",
        "desc": (
            "이 유형은 문제를 풀 때 단순히 공식을 적용하는 것을 넘어, 공식을 직접 유도해보며 개념의 원리부터 파악하려는 성향이 강합니다.\n"
            "또한 어려운 문제를 만나도 끝까지 시도해보려는 끈기와 도전 정신을 갖고 있어, 깊이 있는 사고를 통해 수학을 탐구하려는 태도가 특징입니다."
        ),
        "tip": (
            "먼저 개념을 깊이 있게 이해하는 것이 핵심입니다. 공식을 외우기보다는 증명 과정을 따라 써보며 원리를 스스로 납득해보세요.\n"
    ...
    "LDNA": {
        "name": "📝🐢 꼼꼼 이론가",
        "desc": (
            "공식의 원리와 개념을 철저하게 분석하려는 성향이 있으며, 학습 과정에서 실수를 최소화하려는 꼼꼼한 스타일입니다.\n"
            "하지만 어려운 문제를 마주하면 도전보다는 정확한 문제에 집중하는 편이라 학습 범위 내에서 안정적인 성취를 추구합니다."
        ),
        "tip": (
            "개념을 이해할 때는 작은 정의 하나도 그냥 넘기지 말고 노트에 정리하며 정리한 개념을 반복해서 점검해보는 학습법이 좋습니다.\n"
    ...
    "LDVB": {
        "name": "🧱📊 구조 설계자",
        "desc": (
            "논리적인 원리 이해와 함께 전체 구조를 큰 그림으로 파악하려는 성향이 강합니다.\n"
            "새로운 문제나 낯선 개념도 원리만 이해하면 스스로 체계를 세워 응용할 수 있으며, 어려운 문제도 논리적으로 해결하려는 시도를 멈추지 않습니다."
        ),
        "tip": (
            "개념을 단편적으로 외우기보다는 전체 단원 간 흐름을 파악하고 마인드맵이나 구조도로 연결해보는 방식이 효과적입니다.\n"
    ...
    "LDVA": {
        "name": "🧭📄 흐름 정리러",
        "desc": (
            "공식보다는 전체적인 맥락과 흐름을 파악하며 수학 개념을 이해하려고 하며, 어려운 문제보다는 안정적인 문제풀이를 선호하는 편입니다.\n"
            "개념을 이해한 후 이를 정리하고 체계화하는 데 강점이 있습니다."
        ),
        "tip": (
            "단원을 공부할 때는 먼저 전체적인 개요를 잡은 후, 각 개념을 키워드 중심으로 요약 정리하는 것이 좋습니다.\n"
    ...
    "LFNB": {
        "name": "⚡🔍 빠른 전략가",
        "desc": (
            "공식을 빠르게 적용해보며 문제 풀이에 속도를 내는 데 능숙하고, 동시에 개념을 세부적으로 파악하려는 분석력도 갖추고 있습니다.\n"
            "이런 유형은 수학적 직관과 전략을 동시에 활용해 고난도 문제도 겁내지 않고 시도하는 특징이 있습니다."
        ),
        "tip": (
            "문제를 풀기 전, 어떤 전략이 가장 효율적인지를 빠르게 판단하고 적용해보는 전략 중심 학습이 잘 맞습니다.\n"
    ...
    "LFNA": {
        "name": "🚀📚 빠른 이해러",
        "desc": (
            "개념을 빠르게 받아들이는 능력이 뛰어나며, 개념을 세부적으로 쪼개 분석해 이해하는 경향이 있습니다.\n"
            "하지만 난이도가 높은 문제나 복잡한 응용에는 비교적 부담을 느끼는 경우도 있습니다."
        ),
        "tip": (
            "개념을 빠르게 익힌 후, 다양한 기초 유형 반복 학습을 통해 안정감을 높이는 것이 중요합니다.\n"
    ...
    "LFVB": {
        "name": "🌪️🧩 직관 천재",
        "desc": (
            "수학적 구조나 문제의 틀을 직관적으로 빠르게 파악하고, 복잡한 문제에도 겁내지 않고 도전하는 스타일입니다.\n"
            "머릿속에서 전체 흐름을 이미지처럼 잡는 능력이 있으며, 패턴 인식력이 뛰어납니다."
        ),
        "tip": (
            "복잡한 문제나 창의 문제가 적성에 잘 맞으므로, 수학 올림피아드 문제나 고난도 문제집에 도전해보세요.\n"
    ...
    "LFVA": {
        "name": "🏃‍♂️📘 스피드 마스터",
        "desc": (
            "문제를 빠르게 해결하고자 하며, 필요한 개념만 간단히 익힌 후 실전 문제로 바로 넘어가는 스타일입니다.\n"
            "복잡하고 어려운 문제보다는 시간 안에 정확하게 풀 수 있는 문제에 집중하는 경향이 있습니다."
        ),
        "tip": (
            "시험 직전 요약 정리본과 기출문제를 반복하며 실전 감각을 키우는 것이 핵심입니다.\n"
    ...
    "RDNB": {
        "name": "🧾🔎 공식 분석가",
        "desc": (
            "이 유형은 공식을 빠르게 떠올리고 문제에 바로 적용해보는 습관이 있으며, 문제 풀이 속도가 빠른 편입니다.\n"
            "하지만 단순 계산만이 아니라 원리를 어느 정도 이해하고 응용해보는 걸 즐기며, 복잡한 문제도 끝까지 도전해보려는 태도를 가지고 있어 실전형 수학 능력을 갖추고 있습니다."
        ),
        "tip": (
            "많은 문제를 빠르게 풀고 실수한 문제는 반드시 복기하며, 오답노트 정리를 철저히 해보세요.\n"
    ...
    "RDNA": {
        "name": "📘🛡️ 정석 추종자",
        "desc": (
            "문제를 빠르게 접근하되 실수를 줄이기 위해 세심하게 검토하며 푸는 스타일입니다.\n"
            "그러나 복잡한 문제나 난해한 접근이 필요한 문제는 피하는 경향이 있어, 학습 범위 내에서 정확성을 추구합니다."
        ),
        "tip": (
            "풀이 속도와 정확성의 균형을 맞추는 훈련이 필요합니다. 계산 실수를 줄이려면 반복 훈련과 검산 습관이 중요해요.\n"
    ...
    "RDVB": {
        "name": "🧰📈 유형 설계자",
        "desc": (
            "속도가 빠르면서도 개념을 전체 구조나 논리 흐름으로 이해하려는 특징이 있습니다.\n"
            "어려운 문제도 회피하지 않고 도전하며, 문제 해결에 있어 전략적인 접근을 잘합니다."
        ),
        "tip": (
            "시간 안에 문제를 푸는 훈련과 함께, 각 문제의 접근 전략을 정리해보는 것이 좋습니다.\n"
    ...
    "RDVA": {
        "name": "📂📎 요약 정리왕",
        "desc": (
            "전체 흐름을 빠르게 파악하고 문제를 감각적으로 접근하는 스타일입니다.\n"
            "복잡한 문제보다는 직관적이고 빠른 해결이 가능한 문제를 선호하며, 실전에 강한 유형입니다."
        ),
        "tip": (
            "시험장에서 흔히 나오는 문제들을 중심으로 빈출 유형을 반복 학습하면 성과가 빠르게 올라갑니다.\n"
    ...
    "RFNB": {
        "name": "🥊🧮 실전 파이터",
        "desc": (
            "공식을 빠르게 적용해 문제를 푸는 속도와 정확성을 동시에 갖추고 있으며,\n"
            "개념을 분석하여 응용하는 능력도 뛰어납니다. 어려운 문제에도 적극적으로 도전하며,\n"
            "실전 감각이 매우 강합니다."
        ),
        "tip": (
            "실전 시험처럼 시간 제한 훈련을 꾸준히 하고, 다양한 유형의 고난도 문제에 도전하세요.\n"
    ...
    "RFNA": {
        "name": "🏃‍♀️🔢 공식 스프린터",
        "desc": (
            "공식이나 문제 해결 방식을 빠르게 익히는 데 능숙하며,\n"
            "실수를 줄이기 위해 반복적으로 훈련하는 것을 선호합니다.\n"
            "그러나 복잡한 문제에는 부담을 느껴, 실전 문제 중심으로 학습합니다."
        ),
        "tip": (
            "자주 출제되는 공식이나 유형만 따로 정리한 단권화 노트를 만들어 반복 학습하세요.\n"
    ...
    "RFVB": {
        "name": "⚡🧠 직관 해결사",
        "desc": (
            "문제의 구조를 빠르게 파악하고 직관적으로 해석하는 능력이 뛰어납니다.\n"
            "어려운 문제도 회피하지 않고 풀어나가며, 분석적인 사고보다는 감각적인 접근을 더 선호합니다."
        ),
        "tip": (
            "기출 분석을 통해 문제 구조를 익히고 유사한 문제를 빠르게 푸는 훈련을 해보세요.\n"
    ...
    "RFVA": {
        "name": "🎯📊 시험 만렙러",
        "desc": (
            "요점 중심의 학습을 좋아하며, 시험에 자주 나오는 개념만 정리해 실전에서\n"
            "최대한 성과를 내고자 하는 실용적인 학습자입니다. 응용이나 창의적 사고보다는 효율성과 실전에 강합니다."
        ),
        "tip": (
            "요약본 + 기출 위주 학습이 가장 잘 맞는 유형입니다.\n"
